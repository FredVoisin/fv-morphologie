...
(defmethod transcode ((seq list) (symlist list) &optional (test #'eq))
  (loop until (null seq)
        collect (let* ((a (pop seq))
                  ...
                  (if b (cadr b) a))))
#|
(defmethod transcode ((seq list) (symlist list) &optional (test #'eq) (if-not #'identity))
  (loop until (null seq)
     collect (let* ((a (pop seq))
                       (b (assoc a symlist :test test)))
	       (if b
...
(defmethod alpha>num ((char string) &optional (mode nil))
  (case mode
...
;;; SOME CONVERSIONS (lists, strings, char...)

(defun split-string (string marks)
  ;; marks + space + tabulation : (code2string 9)...
  (if marks
      (let ((pos (findpos (append (string2code marks) (loop for i from 0 to 32 collect i))
			   (string2code string)))
	    words)
	(if pos
...
(defmethod string2sym ((s string))
  (if (> (length s) 0)
      (read-from-string s t nil :preserve-whitespace t) ;:end (length s))
    nil))
